    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white p-6 shadow-sm dark:bg-gray-800 sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('todos.update', $todo) }}" class="w-full space-y-4 sm:w-1/2">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="w-full">
                        ...
                        <textarea id="description" name="description" rows="8" placeholder="More info..."
                            class="block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">{{ $todo->description }}</textarea>
                        @error('description')
                            <p class="mt-2 text-xs text-red-500 dark:text-neutral-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex flex-wrap justify-start gap-6">
                        <ul>
                            @foreach ($tags as $tag)
                                <li>
                                    <input type="checkbox" name="expense_tags[]" value="{{ $tag }}" {{ in_array($tag, $todo->tags->tag) ? 'checked' : '' }}>
                                    {{ $tag }}
                                </li>
                            @endforeach
                        </ul>
