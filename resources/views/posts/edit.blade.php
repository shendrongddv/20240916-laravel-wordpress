    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white p-6 shadow-sm dark:bg-gray-800 sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('posts.update', $post) }}" class="w-full space-y-4 sm:w-1/2">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="w-full">
                        ...
                        <textarea id="content" name="content" rows="8" placeholder="Post content..."
                            class="block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">{{ $post->content }}</textarea>
                        @error('content')
                            <p class="mt-2 text-xs text-red-500 dark:text-neutral-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="is_published" name="is_published" value="{{ $post->is_published }}"
                        ...
                        <label for="is_published" class="ms-3 text-sm text-gray-500 dark:text-neutral-400">Published
                            {{ $post->is_published ? 'YES' : 'NO' }}</label>
                    </div>
                    ...
                    <div class="flex gap-x-6">
                        <div class="flex">
                            <input type="radio" name="status" value="draft"
                                ...
                                id="draft" {{ $post->status === 'draft' ? 'checked' : '' }}>
                        ...
                        <div class="flex">
                            <input type="radio" name="status" value="pending"
                                class="mt-0.5 shrink-0 rounded-full border-gray-200 text-blue-600 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-800 dark:checked:border-blue-500 dark:checked:bg-blue-500 dark:focus:ring-offset-gray-800"
                                id="pending" {{ $post->status === 'pending' ? 'checked' : '' }}>
                            <label for="pending"
                        ...
                        <div class="flex">
                            <input type="radio" name="status" value="published"
                                class="mt-0.5 shrink-0 rounded-full border-gray-200 text-blue-600 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-800 dark:checked:border-blue-500 dark:checked:bg-blue-500 dark:focus:ring-offset-gray-800"
                                id="published" {{ $post->status === 'published' ? 'checked' : '' }}>
                            <label for="published"
                                class="ms-2 text-sm text-gray-500 dark:text-neutral-400">Published</label>
                        </div>
                    </div>
